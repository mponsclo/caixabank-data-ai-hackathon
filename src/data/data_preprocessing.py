...
users_data['birth_date'] = pd.to_datetime(users_data['birth_year'].astype(str) + '-' + users_data['birth_month'].astype(str) + '-01')
...
cards_data['expiry_date'] = pd.to_datetime(cards_data['expiry_year'].astype(str) + '-' + cards_data['expiry_month'].astype(str) + '-01')
...
def preprocess_and_save(df: pd.DataFrame, df_name: str, duplicate_columns: list, outlier_columns: list, output_path: str) -> None:
  """
...
# Example usage
preprocess_and_save(users_data, 'users_data', ['id'], ['current_age', 'yearly_income'], './data/processed/users_data_processed.csv')
preprocess_and_save(cards_data, 'cards_data', ['id', 'client_id'], ['credit_limit'], './data/processed/cards_data_processed.csv')
preprocess_and_save(transactions_data, 'transactions_data', ['id', 'client_id', 'card_id'], ['amount'], './data/processed/transactions_data_processed.csv')
