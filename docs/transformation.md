...
graph TD
  ...
  subgraph Staging["staging (cleaned views)"]
    direction LR
    STG_T["stg_transactions<br/><i>parse errors, normalize</i>"]
    STG_U["stg_users<br/><i>clean $ amounts</i>"]
    STG_C["stg_cards<br/><i>parse chip, expiry</i>"]
    STG_M["stg_mcc_codes<br/><i>normalize codes</i>"]
  end

  subgraph Intermediate["intermediate (enriched views)"]
    INT_TE["int_transactions_enriched<br/><i>joins: txn + MCC + card + user</i>"]
    INT_CT["int_client_transactions"]
  end

  subgraph Marts["presentation (mart tables)"]
    FRAUD["mart_fraud_features<br/><b>60+ features</b><br/><i>window functions</i>"]
    EXPENSES["mart_client_monthly_expenses<br/><i>monthly aggregation</i>"]
  end
...
- **Parses error flags**: The raw `errors` column is a comma-separated string (e.g., `"Bad CVV,Technical Glitch"`). The staging model parses it into 7 boolean columns:
  - `has_bad_cvv`, `has_bad_expiration`, `has_bad_card_number`, `has_bad_pin`
  - `has_insufficient_balance`, `has_technical_glitch`, `has_any_error`
- **Derives channel**: `is_online = 1` when `use_chip = 'Online Transaction'`
- **Casts types**: `zip` from FLOAT to STRING

### stg_users, stg_cards, stg_mcc_codes

Clean demographic and reference data: parse `$`-prefixed currency strings to FLOAT64, normalize boolean fields (`"YES"/"NO"` → BOOLEAN), extract dates from `MM/YYYY` strings.
...
Client-level aggregation of transaction history, used for behavioral features.

## Marts Layer

### mart_fraud_features (60+ columns)

The centerpiece -- [`dbt/models/marts/mart_fraud_features.sql`](../dbt/models/marts/mart_fraud_features.sql) (377 lines). Computes 60+ features across 8 categories, all in SQL via window functions:
...
**Window functions with RANGE BETWEEN for velocity features:**

BigQuery's `RANGE BETWEEN` requires a numeric ORDER BY column. Timestamps don't work directly, so we convert to epoch seconds:
...
This counts transactions per card in the last hour, computed entirely in SQL without Python.

**Correlated subqueries for COUNT(DISTINCT):**

BigQuery doesn't support `COUNT(DISTINCT)` as an analytic function. For features like "distinct MCC codes used by this card in the last 7 days", we use correlated subqueries:

```sql
(SELECT COUNT(DISTINCT mcc) FROM base b2
 WHERE b2.card_id = base.card_id
   AND b2.transaction_date BETWEEN
       TIMESTAMP_SUB(base.transaction_date, INTERVAL 7 DAY)
       AND base.transaction_date
) AS card_distinct_mcc_7d
```

This is computationally expensive on 13M rows -- in production, you'd use approximate COUNT (HyperLogLog) or a feature store.

**Client home state from transaction mode:**

The `is_out_of_home_state` feature computes each client's "home state" as their most frequent `merchant_state`, then flags transactions in other states. This uses a CTE with `ROW_NUMBER()` partitioned by client, ordered by count descending.

### mart_client_monthly_expenses

A simpler mart for expense forecasting (Task 4). Aggregates per client per month:
...
BigQuery was chosen for production because:
1. Scales to billions of rows without memory concerns
2. Integrates with the GCP ecosystem (Cloud Run, Cloud Functions, IAM)
3. dbt-bigquery handles schema management, incremental builds, and data tests
4. Window functions on 13M rows complete in seconds (DuckDB would be fine too, but BigQuery is the portfolio signal)
